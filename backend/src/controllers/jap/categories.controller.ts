// JaP — Categories Controller
// Handles /api/jap/categories* — plant code enforced by plantGuard (PLT-02) + JWT.
import { Request, Response } from "express";
import * as store from "../../data/store";
import { asyncHandler } from "../../middleware/errorHandler";

/** GET /api/jap/categories */
export const listCategories = asyncHandler(async (req: Request, res: Response) => {
  const categories = store.getCategories(req.user!.plantCode);
  res.json(categories);
});

/** POST /api/jap/categories — admin only */
...
/** DELETE /api/jap/categories/:id — admin only */
